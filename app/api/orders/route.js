...
export async function GET(request) {
  try {
    ...
    const orders = await db.collection('orders').find(query).sort({ createdAt: -1 }).limit(limit).toArray();
    return NextResponse.json({ ok: true, orders: serializeDoc(orders) });
  } catch (error) {
...
export async function POST(request) {
  try {
    ...
    const total = money(Math.max(subTotal - discountAmount + serviceCharge + tax, 0));
    ...
    const bulkOps = items
      ...
      .map((item) => ({
        updateOne: {
          filter: { _id: toObjectId(item.productId), trackStock: true },
          update: { $inc: { stock: -item.qty }, $set: { updatedAt: new Date() } }
        }
      }));
    if (bulkOps.length) await db.collection('products').bulkWrite(bulkOps, { ordered: false });

    return NextResponse.json({ ok: true, order: serializeDoc({ ...order, _id: result.insertedId }) });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
  }
}
